...
__global__ void rope_kernel(T* dst, const T* src, const T* cos, const T* sin,
                            uint32_t bh, uint32_t td, uint32_t d,
                            uint32_t stride_b, uint32_t seq_len) {
  const uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  const uint32_t total = bh * td;
  if (idx >= total) return;
  const uint32_t row = idx / td;
  const uint32_t col = idx % td;
  const uint32_t token = row % seq_len;
  ...
  const uint32_t table_idx = token * half_d + (col % half_d);
  const float c = cuda_kernels::to_float(cos[table_idx]);
  const float s = cuda_kernels::to_float(sin[table_idx]);
...
static void launch_rope(void* dst, const void* src, const void* cos, const void* sin,
                        uint32_t bh, uint32_t td, uint32_t d,
                        uint32_t stride_b, uint32_t seq_len, cudaStream_t stream) {
  ...
  rope_kernel<T><<<blocks, threads, 0, stream>>>(static_cast<T*>(dst), static_cast<const T*>(src),
                                                 ...
                                                 bh, td, d, stride_b, seq_len);
}

extern "C" void rope_fwd(void* dst, const void* src, const void* cos,
                          const void* sin, uint32_t bh, uint32_t td, uint32_t d,
                          uint32_t stride_b, uint32_t seq_len, int dtype,
                          cudaStream_t stream) {
  switch (dtype) {
    case 0: launch_rope<__nv_bfloat16>(dst, src, cos, sin, bh, td, d, stride_b, seq_len, stream); break;
    case 1: launch_rope<half>(dst, src, cos, sin, bh, td, d, stride_b, seq_len, stream); break;
    case 2: launch_rope<float>(dst, src, cos, sin, bh, td, d, stride_b, seq_len, stream); break;
  }
}
