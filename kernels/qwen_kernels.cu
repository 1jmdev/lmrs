...
extern "C" __global__ void qwen_fused_silu_mul_f16(
    ...
    const int tid = threadIdx.x;
    const int block_size = blockDim.x;
    const __half *gate = gate_up + row * 2 * intermediate_size;
    const __half *up = gate + intermediate_size;
    __half *out = dst + row * intermediate_size;
    for (int i = tid; i < intermediate_size; i += block_size) {
        const float g = __half2float(gate[i]);
        const float u = __half2float(up[i]);
        out[i] = __float2half(fast_silu(g) * u);
    }
}

extern "C" __global__ void qwen_fused_silu_mul_f32(
    ...
    const int tid = threadIdx.x;
    const int block_size = blockDim.x;
    const float *gate = gate_up + row * 2 * intermediate_size;
    const float *up = gate + intermediate_size;
    float *out = dst + row * intermediate_size;
    for (int i = tid; i < intermediate_size; i += block_size) {
