...
pub fn load_model(model: &str, revision: Option<&str>, device: &Device) -> Result<LoadedModel> {
    ...
    Ok(LoadedModel { config_path, tokenizer_path, tokenizer_config_path, chat_template_path, var_builder: vb })
}
...
fn load_from_dir(root: &Path, device: &Device) -> Result<LoadedModel> {
    ...
    Ok(LoadedModel {
        config_path,
        tokenizer_path,
        tokenizer_config_path: tokenizer_config_path.exists().then_some(tokenizer_config_path),
