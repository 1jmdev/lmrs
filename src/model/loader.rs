...
pub fn load_model(model: &str, revision: Option<&str>, device: &Device) -> Result<LoadedModel> {
    ...
    let api = Api::new()?;
    let repo = api.repo(Repo::with_revision(model.to_owned(), RepoType::Model, revision.unwrap_or("main").to_owned()));
    let config_path = repo.get("config.json")?;
    let tokenizer_path = repo.get("tokenizer.json")?;
    let chat_template_path = repo.get("chat_template.jinja").ok();
    let weights = repo.get("model.safetensors").or_else(|_| repo.get("pytorch_model.bin"))?;
    let vb = unsafe { VarBuilder::from_mmaped_safetensors(&[weights], DType::F16, device)? };
    Ok(LoadedModel { config_path, tokenizer_path, chat_template_path, var_builder: vb })
}
...
fn load_from_dir(root: &Path, device: &Device) -> Result<LoadedModel> {
    ...
    if !config_path.exists() || !tokenizer_path.exists() || !weights.exists() {
        return Err(AppError::BadRequest(format!("model directory must contain config.json, tokenizer.json, and model.safetensors: {}", root.display())));
    }
    fs::metadata(&weights)?;
    let vb = unsafe { VarBuilder::from_mmaped_safetensors(&[weights], DType::F16, device)? };
    Ok(LoadedModel { config_path, tokenizer_path, chat_template_path: chat_template_path.exists().then_some(chat_template_path), var_builder: vb })
}
