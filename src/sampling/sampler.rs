...
impl Sampler {
    ...
    pub fn sample(&mut self, logits: &Tensor) -> crate::error::Result<u32> {
        ...
        for logit in &mut logits { *logit /= self.temperature; }
        let mut scored = logits.into_iter().enumerate().collect::<Vec<_>>();
        scored.sort_by(|a, b| b.1.total_cmp(&a.1));
        if let Some(top_k) = self.top_k { scored.truncate(top_k.max(1)); }
        let max = scored.first().map(|(_, v)| *v).unwrap_or(0.0);
        let mut probs = scored.into_iter().map(|(id, v)| (id, (v - max).exp())).collect::<Vec<_>>();
        let sum: f32 = probs.iter().map(|(_, p)| *p).sum();
        if sum > 0.0 { for (_, p) in &mut probs { *p /= sum; } }
...
fn argmax(values: &[f32]) -> usize {
    values.iter().enumerate().max_by(|a, b| a.1.total_cmp(b.1)).map(|(id, _)| id).unwrap_or(0)
}
